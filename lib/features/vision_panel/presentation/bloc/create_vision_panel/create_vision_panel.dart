...
extension ActionType on VisionView {
  String toName() {
    return switch (this) {
      VisionView.create => 'Create',
      VisionView.edit => 'Submit',
      VisionView.completed => 'Submitted',
    };
  }
}
...
class CreateVisionPanelCubit extends AppBaseCubit<CreateVisionPanelState> {
  ...
  void onValueChanged({
    ...
    final newForm = form.copyWith(
      ...
      totalBoxes: totalBoxes ?? form.totalBoxes,
  ...
  void save() async {
    ...
    return validation.fold(() async {
      ...
      final nextMode = switch (state.view) {
        VisionView.create => VisionView.edit,
        VisionView.edit ||
        VisionView.completed => VisionView.completed,
      };

      final status = switch (state.view) {
        VisionView.create => 'Draft',
        VisionView.edit || VisionView.completed => 'Submitted',
      };

      if (state.view == VisionView.create) {
        final response = await repo.createVision(state.form,state.lines);
...
class CreateVisionPanelState with _$CreateVisionPanelState {
  const factory CreateVisionPanelState({
    ...
    @Default([]) List<VisionItems> newLines,
     required List<VisionItems> lines,
